...
if 'BobPersonality' not in globals():
...
if 'MarkPersonality' not in globals():
...
#     st.session_state.messages.append({"role": "Bob", "content": "Hello, I\'m Bob! Ready to find your perfect electric car? Tell me, what are you looking for? Are you more about speed or range? How can I assist you today?"})
...
if text_task := st.chat_input("Your message"):
    st.session_state.messages.append({"role": "user", "content": text_task})
    with st.chat_message("user"):
        st.markdown(text_task)
    ...
    if any(keyword in text_task.lower() for keyword in keywords):
        ...
        mark_output = f"Mark: {task_mark.output.result}"
        bob_output = f"Bob: {task_bob.output.result}"

        with st.chat_message("assistant"):
            st.session_state.messages.append({"role": "assistant", "conent": mark_output})
        with st.chat_message("assistant"):
            st.session_state.messages.append({"role": "assistant", "conent": bob_output})
        # print("\n\n\n================================================================\n\n\n")
